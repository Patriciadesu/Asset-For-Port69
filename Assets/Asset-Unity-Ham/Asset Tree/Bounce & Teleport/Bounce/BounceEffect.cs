...
public class BounceEffect : ObjectEffect
{
    ...
    public override void ApplyEffect(GameObject player)
    {
        if (player != null)
        {
            ...
            // Apply bounce force
            player.GetComponent<Rigidbody>().linearVelocity = Vector3.zero; // Reset current velocity
            player.GetComponent<Rigidbody>().AddForce(finalBounceDirection * bounceForce, ForceMode.Impulse);
